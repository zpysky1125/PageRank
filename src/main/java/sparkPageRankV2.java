...
public class sparkPageRankV2 {
    public static void main(String[] args) throws Exception{
        ...
        JavaPairRDD<String, Iterable<String>> graphTmp = vertex.subtract(fromSet).mapToPair(v->new Tuple2<>(v,v)).union(pairs).groupByKey().cache();
        ...
        while(rankDif > epison) {
            ...
            graph2 = graph2.flatMapToPair(p->{
                ...
                res = res*0.85+0.15+0.85*0.15*addi;
                list.add(addi.toString());
                list.add(res.toString());
                return list;
            });
        ...
        graph3 = vertex.subtract(toSet).mapToPair(v->new Tuple2<>(v, 0.15)).union(graph3).mapToPair(a -> new Tuple2<>(a._2, a._1)).sortByKey().mapToPair(a -> new Tuple2<>(a._2, a._1));
