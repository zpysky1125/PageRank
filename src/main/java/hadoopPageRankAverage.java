...
public class hadoopPageRankAverage {
    public static class initGraphReducer extends Reducer<Text, Text, Text, Text> {
        ...
        protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            else context.write(key, new Text("1.0" + "\t" + "none"));
        }
    }
    ...
    public static class pageRankMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (lineSplit[2].equals("none")) {
                context.write(new Text(lineSplit[0]), new Text(lineSplit[2]));
                return ;
            }
    ...
    public static class pageRankReducer extends Reducer<Text, Text, Text, Text> {
        ...
        protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            res = res * 0.85 + 0.15 + 0.85*context.getConfiguration().getDouble("iteration.addition", 0.0);
     ...
     private static void initGraph(String input, String output) throws Exception{
        ...
        Job job = Job.getInstance(conf, "Hadoop Page Rank Average");
        job.setJarByClass(hadoopPageRankAverage.class);
        job.setMapperClass(hadoopPageRank.initGraphMapper.class);
        job.setReducerClass(hadoopPageRankAverage.initGraphReducer.class);
     ...
     private static void pageRankIteration(String input, String output, Double addition) throws Exception {
        Configuration conf = new Configuration();
        conf.setDouble("iteration.addition", addition);
        Job job = Job.getInstance(conf, "Hadoop Page Rank Average");
        job.setJarByClass(hadoopPageRankAverage.class);
        job.setMapperClass(hadoopPageRankAverage.pageRankMapper.class);
        job.setReducerClass(hadoopPageRankAverage.pageRankReducer.class);
        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(Text.class);
        job.setOutputKeyClass(Text.class);
        job.setOutputValueClass(Text.class);
    ...
    private static double getAddition(String input) throws Exception {
        Double res = 0.0, lineNum = 0.0;
        File file1 = new File(input + "/part-r-00000");
        BufferedReader reader = null;
        Map<String, Double> rank = new HashMap<>();
        String line = null;
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                String[] lineSplit = line.split("\t");
                if (lineSplit[2].equals("none"))
                    res += Double.parseDouble(lineSplit[1]);
            }
            reader.close();
        } catch (IOException e) {
            e.printStackTrace();
     ...
     private static Integer pageRank(String arg, Double epison) throws Exception {
        Double rankDif = 1.0, addition = 0.0;
        int round = 0;
        String input = arg+round, output = arg + (++round);
        while (rankDif > epison) {
            addition = getAddition(input);
            pageRankIteration(input, output, addition);
